...
async def get_my_categories(
    ...
    categories = await category_service.get_my_categories(current_user_id)
    return ApiResponse(message="카테고리 목록을 조회하였습니다.", data=categories)

@router.post("", response_model=ApiResponse[CategoryResponse])
async def create_category(request: CategoryCreate, category_service: Annotated[CategoryService, Depends()], current_user_id: int = Depends(get_current_user_id)):
    category = await category_service.create_category(request, current_user_id)
...
async def update_category(
    ...
    category = await category_service.update_category(category_id, request, current_user_id)
...
async def delete_category(
    ...
    await category_service.delete_category(category_id, current_user_id)
